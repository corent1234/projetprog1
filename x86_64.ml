...
type double_stocker = { (*permet d'écrire les variables pour les doubles à la fin du programme*)
  mutable texte :  string; 
  mutable num : int ;
  }

let immf x ds =
  ds.texte <- ds.texte ^ "var" ^ string_of_int ds.num ^ ":\n\t.double " ^ string_of_float x ^ "\n";
  ds.num <- ds.num + 1;
  !%("var" ^ string_of_int (ds.num - 1));;

let reg2 r = fun fmt () -> fprintf fmt "%s" r
...
let popsd a = movsd (reg2 "(%rsp)") a ++ inline "\taddq $8, %rsp\n"
